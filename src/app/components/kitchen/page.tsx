...
const WS_URL = "ws://192.168.1.7:3001/api/order"; // your backend WebSocket URL
...
export default function KitchenQueue() {
  ...
  const clearQueue = async () => {
    try {
      await fetch(`${API_URL}/api/order/clear`, { method: "DELETE" });
      setQueue([]);
      setNowServing(null);
    } catch (err) {
  ...
  // ✅ Serve whole order (remove from queue)
  const serveOrder = async (orderId: number) => {
    try {
      await fetch(`${API_URL}/api/order/${orderId}`, { method: "DELETE" });
      setNowServing(queue.find((o) => o.id === orderId) || null);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setNowServing(null), 5000); // Clear after 5 minutes
